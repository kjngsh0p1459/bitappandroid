...
class HeaderScrollableAdapter(context: Context) : NewBaseAdapter<String>(context) {
    ...
    class ViewHolderTable(itemView: View) : NewBaseViewHolder<String>(itemView) {
        ...
        companion object {
            @JvmStatic
            val LAYOUT_ID = R.layout.item_freeze_header_table
        }

        override fun bindData(item: String, position: Int) {
            super.bindData(item, position)
            itemView?.tvFreezeHeader?.text = item
        }
    }
}